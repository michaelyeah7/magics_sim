...
class TestKinematics(unittest.TestCase):
    ...
    def test_CalcBodyToBaseCoordinates(self):
        for i in range(1, int(self.model['NB']) + 1):
            ...
            self.assertAlmostEqual(np.sum(np.abs(py_output-oct_output)), 0.0, 6)
    ...
    def test_CalcPointVelocity(self):
        for i in range(1, int(self.model['NB']) + 1):
            ...
            print(np.sum(np.abs(py_output-oct_output)))
    ...
    # def test_CalcWholeBodyCoM(self):
        # #comment this since we don't ahve model[idcomplot]
    ...
    def test_CalcPosVelPointToBase(self):
        ...
        self.assertAlmostEqual(np.sum(np.abs(oct_ouput[0] - py_output[0])), 0.0, 5)
        self.assertAlmostEqual(np.sum(np.abs(oct_ouput[1] - py_output[1])), 0.0, 5)
