...
# Init env and agent
env = MountainCar(render_flag=render_flag) 
hybrid_env = None
agent = Deep_Agent(
             state_size = 2,
             action_size = 1,
...
exp_dir = "mountaincar_experiments_lr_%f_horizon_%d_" % (lr, horizon)+ strftime("%Y-%m-%d %H:%M:%S", gmtime())
os.mkdir(exp_dir)
...
for j in range(episodes_num):
    ...
    if (j % save_interval == 0 and j!=0):
        #for agent loss
        with open(exp_dir + "/mountaincar_svg_params"+ "_episode_%d_" % j + ".txt", "wb") as fp:   #Pickling
        ...
        with open(exp_dir + "/mountaincar_rewards"+ "_episode_%d_" % j + ".txt", "wb") as fp:   #Pickling
            pickle.dump(episode_rewards[1:], fp)
        plt.figure()
        plt.plot(episode_rewards[1:])
        plt.savefig((exp_dir + '/mountaincar_svg_loss_episode_%d_' % j)+ strftime("%Y-%m-%d %H:%M:%S", gmtime()) + '.png')
        plt.close()

        #for value function loss
        with open(exp_dir + "/mountaincar_svg_value_params"+ "_episode_%d_" % j + ".txt", "wb") as fp:   #Pickling
            pickle.dump(agent.value_params, fp)
        plt.figure()
        plt.plot(agent.value_losses)
        plt.savefig((exp_dir + '/mountaincar_svg_agent_value_loss_episode_%d_' % j) + strftime("%Y-%m-%d %H:%M:%S", gmtime()) + '.png')
        plt.close()        
        # #for model loss
